...
class AccuracyReward(RewardInterface[str, dict]):
    ...
    def compute(
        ...
        test_list = test_list if test_list is not None else [[] for _ in range(n)]
        domains = domains if domains is not None else ["math"] * n

        rewards = []

        for completion, answer, tests, domain in zip(
            completions,
            answers,
            test_list,
            domains,
            ...
            if domain == "code" or len(tests) > 0:
                is_correct = CodeVerifier.verify(text, tests)
            else:
                is_correct = MathVerifier.verify(text, answer)
